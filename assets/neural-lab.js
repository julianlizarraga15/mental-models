(() => {
  ...
  function drawNetwork(){
    ...
    const badge=(text,x,y,{font='700 12px ui-monospace',fill='#f8fafc',background='rgba(7,17,31,.94)',border='rgba(148,163,184,.55)',padX=6,height=20}={})=>{c.font=font;c.textAlign='center';c.textBaseline='middle';const width=Math.ceil(c.measureText(text).width)+padX*2,left=x-width/2,top=y-height/2,r=5;c.beginPath();c.roundRect(left,top,width,height,r);c.fillStyle=background;c.fill();c.strokeStyle=border;c.lineWidth=1;c.stroke();c.fillStyle=fill;c.fillText(text,x,y+.5);c.textBaseline='alphabetic';};
    const weight=(a,b,v,label,t=.5)=>{c.beginPath();c.moveTo(...a);c.lineTo(...b);c.strokeStyle=v>=0?'#38bdf8':'#fb7185';c.globalAlpha=.72;c.lineWidth=1+Math.abs(v)*4;c.stroke();c.globalAlpha=1;badge(`${label} ${fmt(v)}`,a[0]+(b[0]-a[0])*t,a[1]+(b[1]-a[1])*t-8);};
    if(p){weight(nodes.x1,nodes.h1,p.w11,'w₁₁');weight(nodes.x2,nodes.h1,p.w12,'w₁₂',.42);weight(nodes.x1,nodes.h2,p.w21,'w₂₁',.58);weight(nodes.x2,nodes.h2,p.w22,'w₂₂');weight(nodes.h1,nodes.o,p.v1,'v₁',.46);weight(nodes.h2,nodes.o,p.v2,'v₂',.54);}
    const node=(at,name,value,color='#152237')=>{c.beginPath();c.arc(...at,38,0,Math.PI*2);c.fillStyle=color;c.fill();c.strokeStyle='rgba(255,255,255,.7)';c.lineWidth=2;c.stroke();c.fillStyle='#f8fafc';c.textAlign='center';c.font='700 13px system-ui';c.fillText(name,at[0],at[1]-8);badge(value,at[0],at[1]+14,{font:'800 13px ui-monospace',padX:7,height:21,background:'rgba(7,17,31,.88)',border:'rgba(248,250,252,.38)'});};
    ...
    if(p){[['b₁',p.b1,370,32],['b₂',p.b2,370,308],['bₒ',p.bo,660,92]].forEach(b=>badge(`${b[0]} ${fmt(b[1])}`,b[2],b[3],{font:'700 12px ui-monospace',height:22,background:'#1f2937'}));}
  }
